...
- Added localized sample datasets (`sample.csv`, `sample.json`, `sample.xlsx`) per locale in `public/files/{lang}/` with native language column headers.
- Dynamically updated footer sample download links to target localized sample files based on active language selection.
